...
import { X, Award, CheckCircle2, UserCheck, AlertTriangle, Printer, HelpCircle, GraduationCap } from 'lucide-react';
    ...
    if (printContent) {
      ...
      if (windowPrint) {
        windowPrint.document.write(`
          <html>
            <head>
              <title>Print Certificate of Competency</title>
              <style>
                body { font-family: 'Inter', sans-serif; background-color: #f8fafc; padding: 40px; margin: 0; }
    ...
    <div id="drawer-backdrop" className="fixed inset-0 z-50 overflow-hidden bg-slate-900/60 backdrop-blur-xs flex justify-end">
      ...
      >
        ...
        <div className="bg-slate-900 text-white p-6 sticky top-0 z-10 flex items-start justify-between border-b border-slate-800">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              {session && (
                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${
                  ...
                  {sessionStatus === 'ON-GOING' ? 'ACTIVE CLASS' : sessionStatus}
                </span>
              )}
            </div>
            ...
            <p className="text-xs text-slate-400 font-medium mt-1">
              Safety Level: <span className="text-sky-400 font-semibold">{course.level}</span> | Instructor-Led Cohort
            </p>
          </div>
          <button 
            id="btn-close-drawer"
        ...
        <div className="p-6 space-y-6 flex-1">
          ...
          <div>
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Syllabus Overview</h2>
          ...
          <div>
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Lesson Modules Checklist</h2>
          ...
          {session ? (
            <div className="border-t border-slate-150 pt-5 space-y-5">
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Scheduled Batch Specifications</h2>
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 text-white grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="text-xs space-y-2">
                  <div>
                    <span className="text-slate-400 block text-[10px]">Session Period</span>
                    <strong className="text-slate-100 font-mono text-xs">{formatDate(session.startDate)} to {formatDate(session.endDate)}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px]">Training Hours</span>
                    <strong className="text-slate-100 font-mono text-xs">{session.startTime} - {session.endTime} (Daily)</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px]">Accreditation Earned</span>
                ...
                <div className="text-xs space-y-2">
                  <div>
                    <span className="text-slate-400 block text-[10px]">Assigned Classroom</span>
                    <strong className="text-slate-100 text-xs block">{session.classroom}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px]">Authorized Instructor</span>
                    <strong className="text-sky-300 text-xs block">{session.instructor}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px]">Class Occupancy status</span>
              ...
              {sessionStatus !== 'COMPLETED' && (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                  <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                    ...
                    Simulated Cohort Enrollment
                  </h3>
                  <p className="text-[11px] text-slate-500 mb-3">
                    Submit the quick operator form to register a candidate trainee. Capacity seat limits will recalculate.
                  </p>
                  ...
                  {enrollSuccess ? (
                    <div className="bg-emerald-50 border border-white p-3 rounded-lg flex items-center gap-2 text-emerald-900 text-xs font-medium mb-1">
                      ...
                      <div>
                        Candidate successfully added to class registry! Seat tally updated to <span className="font-bold">{session.enrolledIds.length} / {session.maxCapacity}</span>.
                      </div>
                    </div>
                  ) : (
                    <form onSubmit={handleEnrollSubmit} className="flex gap-2">
                      <input
                        type="text"
                        placeholder="Trainee Full Name (e.g. Liam Thompson)"
                      ...
                      >
                        {isSubmittingEnroll ? 'Registering...' : isFull ? 'Class Full' : 'Enroll Trainee'}
                      </button>
                    </form>
                  )}
                </div>
                  ...
                  <div className="flex items-start justify-between gap-3 mb-3 border-b border-indigo-200/30 pb-3">
                    <div>
                      <h3 className="text-xs font-bold text-indigo-900 uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        Regulatory Competence Evaluation Quiz
                      </h3>
                      <p className="text-[11px] text-indigo-700/80 mt-0.5">
                        Unlock a printable certificate for {registrantName || 'the student'} by passing this immediate 3-question evaluation test.
                      </p>
                    </div>
                  ...
                  {!quizStarted ? (
                    <div className="text-center py-4">
                      <p className="text-xs text-slate-600 mb-3">
                        Trainees must score 100% to qualify for instant credentials issuance on {course.code}.
                      </p>
                      <button
                        onClick={() => {
                          setQuizStarted(true);
                    ...
                    <div className="text-center py-4 space-y-3">
                      {quizPassed ? (
                        <>
                          ...
                          <h4 className="text-sm font-extrabold text-slate-800">100% Score! Evaluation Cleared</h4>
                          <p className="text-xs text-slate-600">
                            The trainee passed the competency evaluation for <strong>{course.code}</strong>. Certification generated successfully.
                          </p>
                          ...
                          <div className="pt-2 flex justify-center gap-3">
                            <button
                              ...
                              className="px-3 py-1.5 bg-slate-200 text-slate-700 font-semibold rounded-lg text-xs hover:bg-slate-300 transition-colors"
                            >
                              Reset Quiz
                            </button>
                            <button
                        ...
                        <>
                          ...
                          <h4 className="text-sm font-bold text-slate-800">Review Required</h4>
                          <p className="text-xs text-slate-600">
                            Scored {score} / {questions.length} correct. OSHA and Safety rules demand standard perfect scoring for credentials deployment.
                          </p>
                          <button
                            onClick={() => {
                              setCurrentQuestionIdx(0);
                            ...
                            className="px-4 py-1.5 bg-rose-600 text-white hover:bg-rose-700 font-semibold rounded-lg text-xs"
                          >
                            Re-take Evaluation
                          </button>
                        </>
                    ...
                    <div className="space-y-4">
                      ...
                      {selectedAnswer !== null && (
                        <div className="flex justify-end">
                          ...
                          >
                            {currentQuestionIdx + 1 === questions.length ? 'Submit Answers' : 'Proceed Question'}
                          </button>
                        </div>
                      )}
                    </div>
                ...
                {showCertificate && (
                  ...
                  >
                    <button
                      ...
                      className="absolute top-3 right-3 text-amber-900/60 hover:text-amber-950 p-1 bg-amber-100/50 hover:bg-amber-200/55 rounded-full"
                    >
                      <X className="h-4 w-4" />
                    </button>
                    ...
                    <div ref={certificateRef} className="border-4 double border-amber-700/80 p-5 bg-white text-center rounded-lg relative overflow-hidden">
                      ...
                      <p className="text-[9px] text-slate-400 uppercase tracking-wider italic">
                        This document certifies that competent safety evaluation has been authorized on the student
                      </p>

                      <p className="text-lg font-bold text-green-800 font-sans my-3 py-1 bg-green-50/40 rounded italic underline">
                        {registrantName || 'Distinguished Trainee Student'}
                      </p>

                      <p className="text-[10px] text-slate-500 leading-snug max-w-sm mx-auto mb-4">
                        having successfully audited the academic curriculum, attended site hazard maps, passed rigorous compliance standards, and answered evaluation modules for:
                      </p>
                      ...
                      <div className="flex justify-between items-end mt-6 text-[8px] text-slate-500 border-t border-slate-100 pt-4">
                        ...
                        <div className="text-right">
                          <span className="block border-b border-slate-300 w-24 mb-1"></span>
                          <span className="font-bold">Issue Date: {formatDate('2026-06-04')}</span>
                    ...
                    <button
                      ...
                      className="w-full mt-4 text-xs font-bold py-2 bg-amber-700 text-white rounded-lg flex items-center justify-center gap-1.5 hover:bg-amber-800 transition-colors"
                    >
            ...
            <div className="border-t border-slate-150 pt-5 text-center">
              <p className="text-xs text-slate-500 italic">No scheduled session selected. Viewing general outline only.</p>
            </div>
          )}
        </div>
      </motion.div>
