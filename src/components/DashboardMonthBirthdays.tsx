...
export default function DashboardMonthBirthdays({ members, activeMonth, activeYear }: DashboardMonthBirthdaysProps) {
  ...
  return (
    <div id="dashboard-month-birthdays" className="bg-white border border-slate-200 rounded-xl p-3.5 shadow-sm space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-xs font-black text-slate-900 tracking-tight flex items-center gap-1.5">
              <span>Birthdays in {activeMonth}</span>
              {birthdayMembers.length > 0 && (
                <span className="inline-flex items-center gap-0.5 bg-emerald-100 text-emerald-800 text-[8px] font-bold px-1.5 py-0.5 rounded-full border border-emerald-250 animate-pulse">
                  <Sparkles className="h-2 w-2 animate-spin duration-3000" />
                  {birthdayMembers.length} Celebrate
                </span>
              )}
            </h3>
          </div>
      ...
      {birthdayMembers.length === 0 ? (
        <div className="flex items-center gap-2 py-2 px-3 bg-slate-50 border border-slate-200/65 rounded-lg">
          ...
          <div className="leading-tight">
            <span className="text-slate-800 font-extrabold text-xs font-sans">None</span>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
          {birthdayMembers.map(({ member, formattedDob, age, nextBdayStr, daysRemaining, isToday }) => (
            <div 
              key={member.id} 
              className={`p-3 rounded-lg border flex flex-col justify-between transition-all duration-150 relative overflow-hidden group select-default ${
                isToday 
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0 leading-tight">
                    ...
                    <p className="text-[9px] text-slate-400 truncate font-semibold mt-0.5">{member.position}</p>
                  </div>
                </div>
                ...
                <div className="space-y-1 border-t border-slate-100/80 pt-2 text-[10px]">
                  <div className="flex justify-between items-center text-slate-500 font-sans">
                    ...
                    <span className="font-mono font-bold text-slate-700">{formattedDob}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center text-slate-500 font-sans">
                    ...
                    <span className="font-bold text-slate-800">{age} yrs</span>
                  </div>
              ...
              <div className="mt-2 pt-1.5 border-t border-slate-100/60 flex items-center justify-between">
