...
export default function MembershipInformation({
  ...
  // Google Drive Database Cloud state
  const [googleUser, setGoogleUser] = useState<any>(null);
  const [googleToken, setGoogleToken] = useState<string | null>(null);
  const [dbStatus, setDbStatus] = useState<{ sheetId: string | null; jsonId: string | null; folderFound: boolean }>({ sheetId: null, jsonId: null, folderFound: false });
  const [isDbLoading, setIsDbLoading] = useState(false);
  const [syncMessage, setSyncMessage] = useState<{ text: string; type: 'success' | 'error' | 'info' | null }>({ text: '', type: null });
  const [autoSync, setAutoSync] = useState(() => {
    return localStorage.getItem('se_db_auto_sync') !== 'false'; // Default to true
  });

  const checkCloudStatus = async (tokenStr: string) => {
    setIsDbLoading(true);
    const status = await getDatabaseStatus(tokenStr);
    setDbStatus(status);
    setIsDbLoading(false);
    if (status.error) {
      setSyncMessage({ text: `Failed to load database status: ${status.error}`, type: 'error' });
    }
  };
  ...
  const handleGoogleLogin = async () => {
    setIsDbLoading(true);
    setSyncMessage({ text: 'Connecting to Google...', type: 'info' });
    try {
      const result = await googleSignIn();
      if (result) {
        setGoogleUser(result.user);
        setGoogleToken(result.accessToken);
        setSyncMessage({ text: 'Authenticated with Google Account successfully!', type: 'success' });
        await checkCloudStatus(result.accessToken);
      }
    } catch (err: any) {
      console.error(err);
      setSyncMessage({ text: `Google Auth failed: ${err.message || err}`, type: 'error' });
    } finally {
      setIsDbLoading(false);
    }
  };
  ...
  const handleGoogleLogout = async () => {
    ...
    try {
      ...
      setGoogleUser(null);
      setGoogleToken(null);
      setDbStatus({ sheetId: null, jsonId: null, folderFound: false });
      setSyncMessage({ text: 'Cloud Database disconnected.', type: 'info' });
    } catch (err: any) {
      console.error(err);
    } finally {
      setIsDbLoading(false);
  ...
  const handleCreateSheet = async () => {
    ...
    setIsDbLoading(true);
    setSyncMessage({ text: 'Creating Google Spreadsheet...', type: 'info' });
    try {
      const sheetId = await createDatabaseSheet(googleToken, members);
      setDbStatus((prev) => ({ ...prev, sheetId }));
      setSyncMessage({ text: `Successfully created "${SHEET_NAME}" in Google Drive!`, type: 'success' });
    } catch (err: any) {
      console.error(err);
      setSyncMessage({ text: `Failed to create Spreadsheet: ${err.message || err}`, type: 'error' });
    } finally {
      setIsDbLoading(false);
    }
  };
  ...
  const handleCreateJson = async () => {
    if (!googleToken) return;
    setIsDbLoading(true);
    setSyncMessage({ text: 'Creating JSON database...', type: 'info' });
    try {
      const jsonId = await createDatabaseJson(googleToken, members);
      setDbStatus((prev) => ({ ...prev, jsonId }));
      setSyncMessage({ text: `Successfully created "${JSON_NAME}" in Google Drive!`, type: 'success' });
    } catch (err: any) {
      console.error(err);
      setSyncMessage({ text: `Failed to create JSON file: ${err.message || err}`, type: 'error' });
    } finally {
      setIsDbLoading(false);
    }
  };
  ...
  const handlePullDatabase = async () => {
    if (!googleToken) return;
    if (!dbStatus.sheetId && !dbStatus.jsonId) {
      setSyncMessage({ text: 'Cloud database files not found.', type: 'error' });
      return;
    }
    const confirmed = window.confirm('Are you sure you want to pull data from Google Cloud? This will overwrite your current personnel list with the cloud data.');
    if (!confirmed) return;

    setIsDbLoading(true);
    setSyncMessage({ text: 'Fetching and merging personnel list from Google Drive...', type: 'info' });
    try {
      const cloudMembers = await fetchDatabaseMembers(googleToken, dbStatus.sheetId, dbStatus.jsonId);
      if (cloudMembers && cloudMembers.length > 0) {
        if (onSetMembers) {
          onSetMembers(cloudMembers);
        }
        setSyncMessage({ text: `Successfully downloaded ${cloudMembers.length} personnel records from Cloud!`, type: 'success' });
      } else {
        setSyncMessage({ text: 'No records found to pull from.', type: 'info' });
      }
    } catch (err: any) {
      console.error(err);
      setSyncMessage({ text: `Failed to pull: ${err.message || err}`, type: 'error' });
    } finally {
      setIsDbLoading(false);
    }
  };
  ...
  const handlePushDatabase = async () => {
    if (!googleToken) return;
    if (!dbStatus.sheetId && !dbStatus.jsonId) {
      setSyncMessage({ text: 'Please create spreadsheet and JSON database files first.', type: 'error' });
      return;
    }
    const confirmed = window.confirm('Are you sure you want to push and overwrite the cloud files with your current personnel list?');
    if (!confirmed) return;

    setIsDbLoading(true);
    setSyncMessage({ text: 'Synchronizing local personnel to Google Drive...', type: 'info' });
    try {
      if (dbStatus.sheetId) {
        await writeMembersToSheet(googleToken, dbStatus.sheetId, members);
      }
      if (dbStatus.jsonId) {
        await writeMembersToJson(googleToken, dbStatus.jsonId, members);
      }
      setSyncMessage({ text: 'Synchronized local copy to both Google Sheet & JSON database in your Drive!', type: 'success' });
    } catch (err: any) {
      console.error(err);
      setSyncMessage({ text: `Synchronizing failed: ${err.message || err}`, type: 'error' });
    } finally {
      setIsDbLoading(false);
    }
  };

  // Real-time Automated Push to Google Sheet and .json file when "members" changes
  const prevMembersRef = useRef(members);
  useEffect(() => {
    const runAutoPush = async () => {
      // Deduplicate so it doesn't trigger unless elements truly changed or are loaded
      if (JSON.stringify(prevMembersRef.current) === JSON.stringify(members)) {
        return;
      }
      prevMembersRef.current = members;

      if (autoSync && googleToken && (dbStatus.sheetId || dbStatus.jsonId)) {
        try {
          console.log('[AutoSync] Personnel list changed. Syncing in real-time to Google Drive files...');
          if (dbStatus.sheetId) {
            await writeMembersToSheet(googleToken, dbStatus.sheetId, members);
          }
          if (dbStatus.jsonId) {
          ...
          console.error('[AutoSync] Scheduled auto-push to Google Drive failed:', e);
        }
      }
    };
    
    runAutoPush();
  }, [members, autoSync, googleToken, dbStatus.sheetId, dbStatus.jsonId]);

  const toggleAutoSync = () => {
    const newValue = !autoSync;
    setAutoSync(newValue);
    localStorage.setItem('se_db_auto_sync', String(newValue));
  };
    ...
    <div id="membership-info-container" className="space-y-6 w-full max-w-7xl mx-auto">
      ...
      {activeSection === 'membership' && (
        ...
        <div className="w-full space-y-6 animate-in fade-in duration-200">
          ...
          {/* GOOGLE DRIVE DATABASE CLOUD SYNC WIDGET */}
          <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-[#102d26] text-white p-5 rounded-2xl border border-slate-800 shadow-md">
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6">
              {/* Left Side: Status Info */}
              <div className="space-y-2.5 flex-1">
                <div className="flex items-center gap-2">
                  <div className="bg-emerald-500/10 border border-emerald-500/20 p-1.5 rounded-lg text-[#549B8C]">
                    <Cloud className="h-4.5 w-4.5" />
                  </div>
                  <div>
                    <h4 className="text-[10px] font-black tracking-wider uppercase text-slate-400">Google Drive & Sheets</h4>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-extrabold text-white">Personnel Directory Cloud Database</span>
                      <span className="bg-[#549B8C]/25 text-[#72CBB9] text-[9px] font-bold px-2 py-0.5 rounded-md border border-[#549B8C]/20 font-mono">
                        Active Database
                      </span>
                    </div>
                  </div>
                ...
                <div className="text-xs text-slate-300">
                  <p className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <a 
                      ...
                      className="text-[#72CBB9] hover:text-[#88decb] inline-flex items-center gap-1 font-bold underline cursor-pointer ml-1 text-xs"
                    >
                      {FOLDER_ID} <ExternalLink className="h-3.5 w-3.5" />
                    </a>
                  </p>
                </div>

                {/* File existence trackers */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                  <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-3 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <FileSpreadsheet className={`h-5 w-5 ${dbStatus.sheetId ? 'text-emerald-400' : 'text-slate-500'}`} />
                      <div>
                        <p className="text-[9px] text-slate-400 font-extrabold font-sans leading-none uppercase">Google Sheet File</p>
                        <p className="text-xs font-bold font-mono text-slate-200 mt-1">{SHEET_NAME}</p>
                      </div>
                    </div>
                    {dbStatus.sheetId ? (
                      <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[9px] font-bold px-2 py-0.5 rounded-full font-mono flex items-center gap-1">
                      ...
                      <button 
                        ...
                        className="bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-extrabold px-3 py-1 rounded-lg transition-all cursor-pointer shadow-sm active:scale-95 disabled:opacity-50"
                      >
                        Create Sheet
                      </button>
                    ) : (
                      <span className="text-slate-500 text-[10px] font-bold">Unauthenticated</span>
                    )}
                  </div>

                  <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-3 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <FileCode className={`h-5 w-5 ${dbStatus.jsonId ? 'text-amber-400' : 'text-slate-500'}`} />
                      <div>
                        <p className="text-[9px] text-slate-400 font-extrabold font-sans leading-none uppercase">JSON File Database</p>
                        <p className="text-xs font-bold font-mono text-slate-200 mt-1">{JSON_NAME}</p>
                      </div>
                    </div>
                    {dbStatus.jsonId ? (
                      <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[9px] font-bold px-2 py-0.5 rounded-full font-mono flex items-center gap-1">
                      ...
                      <button 
                        ...
                        disabled={isDbLoading}
                        className="bg-amber-600 hover:bg-amber-500 text-white text-[10px] font-extrabold px-3 py-1 rounded-lg transition-all cursor-pointer shadow-sm active:scale-95 disabled:opacity-50"
                      >
                        Create JSON
                      </button>
                    ) : (
              ...
              {/* Right Side: Identity, Control, Sync */}
              <div className="flex flex-col justify-between items-stretch lg:items-end gap-3 self-stretch min-w-[250px]">
                {/* User authentication detail */}
                <div className="bg-slate-900/60 border border-slate-850 rounded-xl px-3 py-2 flex items-center justify-between lg:justify-end gap-3">
                  {googleUser ? (
                    <div className="flex items-center gap-2.5 w-full">
                      <div className="w-7 h-7 rounded-full bg-[#549B8C]/20 border border-[#549B8C]/40 text-[#72CBB9] flex items-center justify-center font-bold text-xs shrink-0 overflow-hidden">
                        {googleUser.photoURL ? (
                          <img referrerPolicy="no-referrer" src={googleUser.photoURL} className="w-full h-full object-cover" alt="" />
                        ) : (
                          googleUser.email ? googleUser.email[0].toUpperCase() : 'G'
                        )}
                      </div>
                      <div className="text-left flex-1 min-w-0">
                        <p className="text-[9px] text-slate-400 font-bold font-sans">CONNECTED GOOGLE</p>
                        <p className="text-xs font-bold text-slate-200 truncate leading-none mt-0.5" title={googleUser.email}>{googleUser.email}</p>
                      </div>
                      <button 
                    ...
                    <div className="flex items-center justify-between w-full gap-3">
                      <span className="text-[10px] text-slate-400 font-bold font-sans">DB INTEGRATION CONNECT</span>
                      <button 
                        onClick={handleGoogleLogin}
                        disabled={isDbLoading}
                        className="bg-white hover:bg-slate-100 text-slate-900 border border-slate-200 shadow-2xs text-[10px] font-black px-3 py-1.5 rounded-lg flex items-center gap-1.5 cursor-pointer select-none transition-all active:scale-97"
                      >
                ...
                {/* Pull, Push, Auto switches */}
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full lg:w-auto">
                  <button 
                    onClick={handlePullDatabase}
                    disabled={!googleToken || (!dbStatus.sheetId && !dbStatus.jsonId) || isDbLoading}
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-slate-800 text-slate-200 hover:bg-slate-700 text-xs font-bold border border-slate-700/60 rounded-xl transition-all cursor-pointer disabled:opacity-45 disabled:cursor-not-allowed text-center"
                    title="Retrieve personnel folder database and pull into local view"
                  >
                  ...
                  <button 
                    ...
                    disabled={!googleToken || (!dbStatus.sheetId && !dbStatus.jsonId) || isDbLoading}
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-[#549B8C] hover:bg-[#437C70] text-slate-50 text-xs font-bold rounded-xl transition-all cursor-pointer shadow-xs disabled:opacity-45 disabled:cursor-not-allowed text-center"
                    title="Push current local personnel and overwrite cloud databases"
                  >
                ...
                {googleToken && (dbStatus.sheetId || dbStatus.jsonId) && (
                  <label className="flex items-center justify-between sm:justify-start gap-3 cursor-pointer self-stretch lg:self-auto text-xs py-1 px-1">
                    <span className="text-[10px] text-slate-400 font-extrabold select-none uppercase tracking-wide">
                      Real-time Auto-Push:
                    </span>
                    <button
                      type="button"
                      onClick={toggleAutoSync}
                      className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-hidden ${autoSync ? 'bg-emerald-500' : 'bg-slate-700'}`}
                    >
                      <span
                        className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out ${autoSync ? 'translate-x-4' : 'translate-x-0'}`}
                      />
                    </button>
                  </label>
                )}
            ...
            {/* Sync feedback notification strip */}
            {syncMessage.text && (
              <div className={`mt-4 px-3 py-2.5 rounded-xl border flex items-center justify-between text-xs font-bold leading-tight ${
                ...
                <div className="flex items-center gap-2">
                  {isDbLoading ? (
                    <Loader2 className="h-4 w-4 animate-spin shrink-0 text-[#72CBB9]" />
                  ) : (
                    <Cloud className="h-4 w-4 shrink-0 text-slate-300" />
                  )}
                  <span>{syncMessage.text}</span>
                </div>
                <button 
                  onClick={() => setSyncMessage({ text: '', type: null })}
                  className="text-slate-400 hover:text-white p-1 ml-2 font-black transition-colors"
                >
