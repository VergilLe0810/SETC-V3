...
export default function MembershipInformation({
  ...
  const [autoSync, setAutoSync] = useState(() => {
    return localStorage.getItem('se_db_auto_sync') === 'true';
  });
  ...
  useEffect(() => {
    const runAutoPush = async () => {
      if (autoSync && googleToken && (dbStatus.sheetId || dbStatus.jsonId)) {
        try {
          ...
          console.log('Automated Cloud Sync update pushed successfully.');
        } catch (e) {
    ...
    if (prevMembersLength.current !== members.length) {
      runAutoPush();
    }
